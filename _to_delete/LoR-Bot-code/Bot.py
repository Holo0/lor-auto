...
class Bot:
                ...
                map(
                    ...
                    (
                        ...
                        for edge, msk in zip(
                            cv2.Canny(
                                cv2.cvtColor(
                                    np.array(image.crop(box=(1585, 638, 1635, 675))),
                                    cv2.COLOR_BGR2GRAY,
                                ),
                                100,
                                100,
    ...
    def run(self):
        while True:
            (
                ...
                self.n_games,
                self.games_won,
            ) = self.state_machine.get_game_info()
            ...
            self.mouse_handler.move_mouse_smooth(
    ...
    def play(self):
        in_game_cards = [
            card for cards in self.cards_on_board.values() for card in cards
        ]
        ...
        if self.game_state == GameState.Mulligan:
            ...
            (
                ...
                self.n_games,
                self.games_won,
            ) = self.state_machine.get_game_info(call_game_state=False)
            in_game_cards = [
                card for cards in self.cards_on_board.values() for card in cards
            ]
            ...
            if self.deck_strategy:
                self.deck_strategy.mulligan(
                    in_game_cards, self.window_x, self.window_y, self.window_height
                )
            ...
            block_counter = 0
            while block_counter < 12 and self.deck_strategy.block(
                self.cards_on_board, self.window_x, self.window_y, self.window_height
            ):
                sleep(2)
                (
                    self.game_state,
            ...
            self.game_state == GameState.Defend_Turn
            or self.game_state == GameState.Attack_Turn
        ):
            if len(self.cards_on_board["spell_stack"]) != 0 and all(
                (card.is_spell() or card.is_ability())
                for card in self.cards_on_board["spell_stack"]
            ):
            ...
            playable_cards = sorted(
                filter(
                    lambda card: not (card.is_spell() or card.is_equipment())
                    and card.cost <= self.mana,
                    self.cards_on_board["cards_hand"],
                ),
                key=lambda card: card.cost,
                reverse=True,
            )
            if (
                len(playable_cards) == 0
                and self.game_state == GameState.Attack_Turn
                or len(self.cards_on_board["cards_board"]) == 6
            ):
                keyboard.send("a")
                ...
                while not self.deck_strategy.reorganize_attack(
                    self.cards_on_board,
                    self.window_x,
                    self.window_y,
                    self.window_height,
                ...
                if playable_card_in_hand:
                    ...
                    if "ally in hand" in playable_card_in_hand.description_raw:
                        ...
                        units_in_hand = tuple(
                            filter(
                                lambda card_in_hand: not (
                                    card_in_hand.is_spell()
                                    or card_in_hand.is_equipment()
                                ),
                                self.cards_on_board["cards_attk"],
                            )
                        )  # NOTE: Has to be cards_attk, because they are lifted
                        if units_in_hand:
                            select_epehemral = (
                                "Ephemeral" in playable_card_in_hand.description_raw
                            )
                            card_to_click = self.deck_strategy.get_card_in_hand(
                                units_in_hand, select_epehemral
                            )
                            self.mouse_handler.click(
                                self.window_x + card_to_click.top_center[0],
                                self.window_y
                                + self.window_height
                                - card_to_click.top_center[1],
                            )
                    # Imperial Demolist play effect
                    elif (
                        "to an ally" in playable_card_in_hand.description_raw
                        and len(self.cards_on_board["cards_board"]) != 0
                    ):
                        ...
                        sleep(0.75)
                        self.mouse_handler.click(
                            self.window_x + 4 * self.window_width // 7,
                            self.window_y + self.window_height // 2,
                        )
                        sleep(1)
                    ...
                    if playable_card_in_hand.is_spell():
                        self.spell_mana = max(
                            0, self.spell_mana - playable_card_in_hand.cost
                        )
    ...
    def play_card(self, card):
        (x, y) = (
            self.window_x + card.top_center[0],
            self.window_y + self.window_height - card.top_center[1],
        )
    ...
    def get_display_data(self) -> dict:
        return {
            ...
            "cards_on_board": self.cards_on_board,
            "deck_type": self.deck_type,
            "mana": self.mana,
            "spell_mana": self.spell_mana,
            "prev_mana": self.prev_mana,

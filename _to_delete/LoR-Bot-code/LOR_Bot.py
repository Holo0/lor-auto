...
def _make_background(shape, color):
    """Create a background image in 8-bit BGR format compatible with OpenCV."""
    image = np.zeros((*shape, len(color)), dtype=np.uint8)
    normalized_color = []
    for channel in color:
        if isinstance(channel, float):
            normalized_color.append(int(round(max(0.0, min(1.0, channel)) * 255)))
        else:
            normalized_color.append(int(max(0, min(255, channel))))
    image[:] = normalized_color
    return image

background_info = _make_background((720, 1280), (0.0, 0.0, 0.0))
...
while True:
    ...
    background_info = _make_background(shape, background_color)
